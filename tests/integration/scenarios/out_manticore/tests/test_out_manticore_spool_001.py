...
from test_out_manticore_001 import (
    read_request,
    read_service_log,
    send_response,
)
...
class SpoolRecoveryTests(unittest.TestCase):
    ...
    def stop_service(self, service):
        ...
        service.stop()
        return process.returncode, read_service_log(service)
    ...
    def run_session(self, label, records, env=None, signal_after_spool=False):
        ...
        if records and not signal_after_spool:
        ...
        session_env = {
            ...
            "MANTICORE_EXIT_ON_EOF": "false",
        }
        if env:
            session_env.update(env)
        (self.root / (label + "-config.txt")).write_text(str(CONFIG_FILE))
        expected_request_count = (
            len(self.receiver.requests) + 1 if not records else None
        )
        service = FluentBitTestService(str(CONFIG_FILE), extra_env=session_env)
        try:
            service.start()
        except FluentBitStartupError:
            output = read_service_log(service)
            (self.root / (label + ".log")).write_text(output)
            (self.root / (label + "-exit.txt")).write_text("1")
            return ServiceResult(1, output)
        ...
        if signal_after_spool:
            ...
            while not self.spool.exists() and process.poll() is None:
                if time.monotonic() >= deadline:
                    service.stop()
                    self.fail("Manticore spool was not populated\n{}".format(
                        read_service_log(service)))
                time.sleep(0.1)
            if process.poll() is not None:
                output = read_service_log(service)
                service.stop()
                self.fail("Fluent Bit exited before SIGTERM\n{}".format(output))
        returncode, output = self.stop_service(service)
        if not records:
            deadline = time.monotonic() + 30
            while len(self.receiver.requests) < expected_request_count:
                if time.monotonic() >= deadline:
                    self.fail("Manticore request was not received\n{}".format(
                        output))
                time.sleep(0.1)
        (self.root / (label + ".log")).write_text(output)
        (self.root / (label + "-exit.txt")).write_text(str(returncode))
        for name, path in [("spool", self.spool), ("journal", self.journal)]:
            if path.exists():
                (self.root / (label + "." + name)).write_bytes(path.read_bytes())
        return ServiceResult(returncode, output)
